...
const DeliveryManagement = () => {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {/* Afficher les tournées disponibles */}
      <div className="mb-6">
        <h2 className="text-xl font-semibold mb-4">Tournées disponibles :</h2>
        {filteredTours.length > 0 ? (
          <ul className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredTours.map((tour) => (
              <li key={tour.ID_Tournee} className="border p-4 rounded-md shadow hover:bg-gray-100">
                <button
                  onClick={() => setSelectedTourId(tour.ID_Tournee)}
      ...
      {/* Afficher la carte pour la tournée sélectionnée */}
      {selectedTourId && (
        <div>
