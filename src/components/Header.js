...
function Header() {
  ...
  return (
    <header className={`bg-transparent text-white py-4 fixed w-full top-0 z-50 transition-transform ${hidden ? '-translate-y-full' : 'translate-y-0'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
          ...
          <ul className="flex space-x-5">
            <li><a href="/" className="hover:text-blue-400">Accueil</a></li>
             {user?.admin === 1 && ( // Vérification si l'utilisateur est admin */}
              <li><a href="/transcribe" className="hover:text-blue-400">Créer tournée livraison</a></li>
            )}
            <li><a href="/tarif" className="hover:text-blue-400">Tarif</a></li>
        ...
        <div className="flex items-center space-x-4">
          ...
          {isAuthenticated ? (
            <button onClick={logout} className="bg-red-600 text-white py-2 px-4 rounded-lg hover:bg-red-700">
              Déconnexion
            </button>
          ) : (
            <>
